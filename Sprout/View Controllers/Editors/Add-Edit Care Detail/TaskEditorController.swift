...
extension TaskEditorController {
    ...
    private func applyFrequencyPickerSnapshot(after section: Section? = nil) {
        ...
        self.dataSource.apply(frequencyPickerSnapshot, to: .recurrenceFrequency, animatingDifferences: false)
    }
...
private extension TaskEditorController {
    func makeDetailHeaderRegistration() -> UICollectionView.CellRegistration<SproutCareDetailCell, Item> {
        UICollectionView.CellRegistration<SproutCareDetailCell, Item> {[unowned self] cell, indexPath, item in
            let task = self.task
